...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, mempool, net, sharedexcoin, (obfuscation, swifttx, masternode, mnpayments, mnbudget)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all ShareDexCoin specific functionality (Masternodes, Obfuscation, SwiftTX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-anonymizesharedexcoinamount=<n>", strprintf(_("Keep N SDC anonymized (default: %u)"), 0));
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(_("Copyright (C) 2018 The ShareDexCoin Core Developers")) + "\n" +
           "\n" +
...
// ********************************************************* Step 8: load wallet
    ...
    nAnonymizeSharedexcoinAmount = GetArg("-anonymizesharedexcoinamount", 0);
    if (nAnonymizeSharedexcoinAmount > 999999) nAnonymizeSharedexcoinAmount = 999999;
    if (nAnonymizeSharedexcoinAmount < 2) nAnonymizeSharedexcoinAmount = 2;
    ...
    LogPrintf("Anonymize ShareDexCoin Amount %d\n", nAnonymizeSharedexcoinAmount);
    ...
    /* Denominations
       ...
       For example:
       1SDC+1000 == (.1SDC+100)*10
       10SDC+10000 == (1SDC+1000)*10
    */

...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizeSharedexcoinAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizeSharedexcoinAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizeSharedexcoinAmount = strAnonymizeSharedexcoinAmount.remove(strAnonymizeSharedexcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSharedexcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizeSharedexcoinAmount * COIN) nMaxToAnonymize = nAnonymizeSharedexcoinAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizeSharedexcoinAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizeSharedexcoinAmount));
        strAnonymizeSharedexcoinAmount = strAnonymizeSharedexcoinAmount.remove(strAnonymizeSharedexcoinAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizeSharedexcoinAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
...
void OverviewPage::toggleObfuscation()
{
    ...
    if (!fEnableObfuscation) {
        ...
        if (nAnonymizeSharedexcoinAmount == 0) {
